# ÍNDICE MAESTRO — Material teórico olímpico (v2)

Inventario completo del temario olímpico para Teoría de Números y Geometría.

- ✅ Cubierto
- ⏳ Pendiente

---

## TEORÍA DE NÚMEROS

### Aritmética elemental (iniciación · regional)

- ✅ Divisibilidad básica, algoritmo de Euclides
- ✅ Paridad e invariantes
- ✅ Congruencias módulo n
- ✅ Euclides–Bézout
- ✅ Bases numéricas y representación en base $b$

### Aritmética modular (regional · nacional)

- ✅ Pequeño teorema de Fermat
- ✅ Teorema Chino del Resto
- ✅ Función $\varphi$ de Euler
- ✅ Teorema de Wilson
- ✅ Orden multiplicativo
- ✅ Raíces primitivas y $(\mathbb Z/p\mathbb Z)^*$
- ✅ Reciprocidad cuadrática
- ⏳ Símbolo de Jacobi

### Estructura algebraica (nacional · internacional)

- ✅ Valuación $p$-ádica
- ✅ Lifting the Exponent (LTE)
- ✅ Polinomios ciclotómicos $\Phi_n(x)$
- ✅ Teorema de Lucas para binomiales
- ✅ Polinomios (Vieta, Newton, Eisenstein)
- ✅ Funciones multiplicativas y convolución de Dirichlet
- ⏳ Caracteres de Dirichlet, sumas de Gauss
- ⏳ Wolstenholme

### Sucesiones y aproximación

- ✅ Sucesiones recurrentes lineales (Fibonacci, Lucas, Pisano)
- ✅ Fracciones continuas y Pell
- ⏳ Teorema de Hensel

### Números especiales

- ✅ Números perfectos y primos de Mersenne
- ⏳ Primos de Fermat, Wieferich

### Teoremas profundos

- ✅ Zsigmondy
- ⏳ Dirichlet (progresiones aritméticas, enunciado y casos)
- ⏳ Bertrand–Chebyshev

### Métodos

- ✅ Descenso infinito
- ✅ Vieta jumping
- ✅ Inducción
- ✅ Palomar
- ✅ Principio del extremo
- ✅ Invariantes
- ✅ Ecuaciones diofánticas
- ⏳ Funciones generadoras
- ⏳ Cribas elementales

### Problemas resueltos / guiados

| Nivel | Disponibles |
|---|---|
| iniciación | ✅ **IMO 1959 P1** (guiado) |
| regional | OMG 2017, 2019, 2020, 2021 + ✅ OMG 2016 (guiado) |
| nacional | OME 2015, 2012 + ✅ OME 2019 (guiado), ✅ **IMO 1988 P6** (guiado) |
| internacional | IMO 1988, 2009, 2008 |
| élite | ISL 2014, USAMO 2010 |

---

## GEOMETRÍA

### Sintética básica (iniciación · regional)

- ✅ Ángulos en la circunferencia
- ✅ Semejanza
- ✅ Potencia de un punto
- ✅ Puntos notables del triángulo
- ✅ Trigonometría del triángulo
- ✅ Stewart
- ✅ Ceva y Menelao

### Configuraciones cíclicas

- ✅ Ptolomeo
- ✅ Simson y Steiner
- ✅ Circunferencia de los nueve puntos
- ✅ Eje radical y centro radical
- ✅ Lema del Tridente
- ✅ Feuerbach
- ✅ Gergonne y Nagel
- ⏳ Casey

### Transformaciones

- ✅ Homotecia
- ✅ Inversión
- ✅ Rotohomotecia (similitud espiral)
- ⏳ Isometrías generales

### Geometría avanzada

- ✅ Polos y polares
- ✅ Razón cruzada y armónicas
- ✅ Pascal y Brianchon
- ✅ Miquel
- ✅ Simedianas e isogonales (Lemoine)
- ⏳ Cónicas en olimpiada

### Métodos

- ✅ Coordenadas (cartesianas, baricéntricas, complejas)
- ✅ Carnot (perpendiculares concurrentes)
- ⏳ Vectores

### Desigualdades

- ✅ Triangular y Euler ($R \geq 2r$)
- ✅ Erdős–Mordell, Weitzenböck, Hadwiger–Finsler
- ✅ Fermat–Torricelli
- ⏳ Pedoe, Leibniz

### Problemas resueltos / guiados

| Nivel | Disponibles |
|---|---|
| regional | OMG 2018, OMG 2023 + ✅ OMG 2014 (guiado) |
| nacional | OME 2012 |
| internacional | IMO 2008 |

---

---

## ÁLGEBRA

### Desigualdades fundamentales (iniciación · regional)

- ✅ Desigualdad AM-GM: casos $n=2$, $n=3$, caso general, forma ponderada
- ✅ Cauchy-Schwarz: forma clásica y forma de Engel/Titu/Sedrakyan
- ✅ Sucesiones y recurrencias lineales: ecuación característica, Fibonacci

### Polinomios (regional · nacional)

- ✅ Vieta, teorema del factor, identidades algebraicas, simétricos elementales
- ⏳ Polinomios ciclotómicos sobre $\mathbb R$
- ⏳ Polinomios de Chebyshev

### Desigualdades intermedias (regional · nacional)

- ✅ Convexidad y desigualdad de Jensen
- ✅ Desigualdad de Schur ($t$ general; forma en $e_1, e_2, e_3$)
- ⏳ Rearrangement inequality
- ⏳ Power mean inequality

### Desigualdades avanzadas (nacional · internacional)

- ✅ Muirhead y mayorización: $[x_1,\ldots,x_n] \geq [y_1,\ldots,y_n]$
- ⏳ Popoviciu, Vornicu–Schur

### Ecuaciones funcionales (nacional · internacional)

- ✅ Tipos principales: Cauchy aditiva, Cauchy multiplicativa, Jensen funcional
- ⏳ Ecuaciones de Pexider
- ⏳ Ecuaciones con condición aritmética (e.g., $f(mn)=f(m)f(n)$)

### Métodos

- ✅ Sustitución y normalización (suma $= 1$, producto $= 1$, Ravi, trigonométrica)
- ✅ Sum of Squares (SOS) y método $uvw$
- ✅ Tangent Line Trick (TLT)
- ✅ Cómo atacar ecuaciones funcionales: sustituciones, inducción, regularidad
- ⏳ Multiplicadores de Lagrange en olympiada
- ⏳ Funciones generatrices (convex-hull trick)

### Demostraciones

- ✅ Tres demostraciones de AM-GM (Cauchy hacia adelante-atrás, convexidad, reordenamiento)
- ✅ Dos demostraciones de Cauchy-Schwarz (discriminante, identidad de Lagrange)

### Problemas resueltos / guiados

| Nivel | Disponibles |
|---|---|
| regional | ✅ Ecuación de Cauchy funcional (guiado) |
| nacional | IMO 1995/P2 (Cauchy-Schwarz) + ✅ IMO 2000/P2 (guiado) |
| internacional | IMO 2001/P2 (raíces), IMO 2010/P1 (funcional) |

---

## Áreas no cubiertas (futuras expansiones)

- **Combinatoria:** grafos, conteo doble, principio extremal, Ramsey, juegos.

---

## Filosofía editorial

Cada entrada teórica sigue la estructura:

1. **Definición** o enunciado preciso.
2. **Demostración** completa cuando es razonable.
3. **Ejemplos** trabajados.
4. **Aplicaciones** que conectan con la práctica.
5. **Problemas relacionados**.

Los problemas guiados van más allá:

- **Fase 1:** leer y entender el enunciado.
- **Fase 2:** experimentar con casos pequeños.
- **Fase 3:** identificar la idea clave.
- **Fase 4:** ejecutar el argumento.
- **Fase 5:** demostración limpia (sin proceso heurístico).
- **Observación:** lección técnica generalizable.

Distinguir entre demostración formal y proceso de pensamiento es lo que
diferencia un texto **didáctico** de uno **enciclopédico**.

---

*Última actualización: febrero 2026.*
